...
- **The MCP handshake now reports the actual package version.** The server was
  constructed with a literal `version: "2.0.0"`, never updated after that
  release — so every client from 2.1.0 through 2.5.0 saw `2.0.0` in the
  `initialize` response. The version is now read from `package.json` instead of
  restated in the source, so a future bump cannot leave it behind. No tool
  behaviour changes.
